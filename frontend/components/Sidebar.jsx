    ...
    <>
      ...
      <div className={`sidebar ${isOpen ? 'open' : ''}`}>
        ...
        <nav className="nav">
          ...
          {role !== 'vendor' && (
            <>
              <Link to="/dashboard" className={`ni ${isNavActive('/dashboard') ? 'on' : ''}`} onClick={onClose}>
        ...
        <div className="sfooter">
          <div className="ava">{role === 'vendor' ? 'V' : 'AE'}</div>
          <div>
            <div className="uname">{role === 'vendor' ? 'Vendor Dapur' : 'Admin Eksekutif'}</div>
            <div className="urole">{role === 'vendor' ? 'SPPG JKT-001' : 'SPPG Pusat'}</div>
          </div>
        </div>
      </div>
    </>
